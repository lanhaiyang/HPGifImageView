...
+ (instancetype)sharedCache {
    ...
    dispatch_once(&once, ^{
        ...
        instance.cacheQueu=dispatch_queue_create("HPCache.queue.gif", NULL);
    });
    return instance;
}
...
+(BOOL)cacheWithObj:(NSData *)data cacheName:(NSString *)cacheName
{
    ...
    if (addFile==YES) {
        ...
        [HPCacehObject.fileSizeNiformation setObject:@(fileSize) forKey:[self PrefixPath:cacheName]];
...
+(NSString *)PrefixPath:(NSString *)chilePatch
{
    NSArray *paths=[chilePatch componentsSeparatedByString:chilePatch];
    if (paths.count>0) {
        return paths[0];
    }
    return chilePatch;
